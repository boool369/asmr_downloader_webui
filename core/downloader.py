...
def recursively_transform_data_v2(
    ...
    for item in data:
        ...
        # 仅处理允许的类型
            ...
            # 如果配置为只下载 HQ 且当前不是 HQ，则跳过
            if hq_only and item_type == "audio" and not is_hq:
                continue
...
async def download_worker(
    ...
    # 清理路径中的非法字符
    safe_folder_path = Path(folder_path_str.replace(":", "：").replace("?", "？"))
            ...
            async with session.get(file_url, headers=download_headers) as response:
                ...
                async with aiofiles.open(full_path, mode) as f:
                    ...
                    async for chunk in response.content.iter_chunked(chunk_size):
                        ...
                        if (current_downloaded // chunk_size) % 50 == 0:
...
async def process_bulk_download_job(
    ...
    for i, rj_id in enumerate(rj_ids):
        ...
        def work_progress_callback(rj_id_local: str, filename: str, downloaded: int, total: int):
            ...
            # 注意：这里只更新文本描述，进度条由 overall_progress_callback 控制作品数
...
async def search_work_async(keyword: str, page: int = 1, size: int = 20) -> Tuple[List[Dict[str, Any]], int]:
    """
    根据关键词搜索作品。
    新增 size 参数，用于自定义每页显示数量。
    返回 (作品信息列表, 总页数)
    """
    await log_message(f"Searching for '{keyword}' on page {page}, size {size}") # 记录 size

    try:
        config = load_config()
        nsfw = True  # 搜索功能默认不限制 NSFW，UI 上没有开关
        ...
        async with aiohttp.ClientSession() as session:
            ...
            works = r["works"]
            # ❗ 修改：使用 size 来计算总页数
            total_pages = ceil(r["pagination"]["totalCount"] / size) if r["pagination"]["totalCount"] and size > 0 else 0
